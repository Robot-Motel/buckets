...
pub fn validate_and_canonicalize_path(
    path: &Path,
    ...
    if let Some(filename) = path.file_name() {
        if filename.len() > MAX_FILENAME_LENGTH {
            return Err(BucketError::SecurityError(format!(
        ...
        let canonical_path = if full_path.exists() {
            ...
            if parent.exists() {
                let canonical_parent = parent.canonicalize().map_err(|e| {
                    BucketError::PathValidationError(format!(
...
mod tests {
    ...
    pub fn validate_path_exists(path: &Path) -> Result<(), BucketError> {
        if !path.exists() {
            return Err(BucketError::PathValidationError(format!(
                "Path does not exist: {}",
                path.display()
            )));
        }
